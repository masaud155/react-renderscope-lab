...
export default function RenderNode({ node, scenario, mode, counters }) {
  ...
  return (
    <div
      className={cn("flex min-w-0 items-center justify-between gap-3 rounded-md border bg-white/[0.03] px-3 py-2", severityStyles[severity])}
      style={{ marginLeft: `${node.depth * 18}px` }}
    >
      <span className="min-w-0 truncate font-mono text-xs text-white">{node.depth ? "L- " : ""}{node.name}</span>
